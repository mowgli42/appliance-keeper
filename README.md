...
Today you can browse a demo household, mark filters changed, attach nameplate/receipt/manual files, see useful-life tips and manufacturer support contacts, and print a PDF-ready dossier.
...
```mermaid
flowchart LR
  UI[SvelteKit UI<br/>static SPA] --> Store[Household store<br/>localStorage v2]
  Store --> Rules[Domain rules<br/>attention · useful life · media]
  UI --> Caps[Capacitor shell<br/>Android]
  Rules --> Export[Printable dossier HTML<br/>Save as PDF]
...
- **Persistence:** on-device only (`localStorage`; media as data URLs in Phase 1)
...
- [ ] README health polish tracked in [#2](https://github.com/mowgli42/appliance-keeper/issues/2) (this PR addresses it)
...
See `openspec/project.md` and archived change `openspec/changes/archive/2026-08-02-add-centriq-style-media-and-insights/`.
